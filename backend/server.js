...
// EM server.js (pode ser perto das outras rotas de recompensas)
...
// EM server.js (junto com as outras rotas de clientes)
...
app.post('/api/rewards/redeem', verifyToken, async (req, res) => {
    try {
        ...
        // 1. Busca os dados da recompensa e os pontos atuais do cliente em uma única consulta
        ...
        // 4. Retorna os dados do produto resgatado para o frontend adicionar ao carrinho
        const [products] = await pool.query("SELECT * FROM products WHERE id = ?", [reward.product_id]);
        if (products.length === 0) {
            // Isso pode acontecer se o produto foi deletado mas a recompensa não
...
    try {
        // A consulta SQL agora busca também a coluna 'points'
