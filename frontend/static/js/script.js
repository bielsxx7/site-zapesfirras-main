document.addEventListener('DOMContentLoaded', () => {
    ...
    const atualizarTodosResumos = () => {
        const subtotal = carrinho.reduce((acc, item) => {
            const precoItemTotal = parseFloat(item.price) + (item.adicionais ? item.adicionais.reduce((sum, ad) => sum + ad.preco, 0) : 0);
        ...
        const resumoHTML = `
            ...
            <div class="linha-resumo"> <span>Taxa de entrega</span> <span>R$ ${taxaEntrega.toFixed(2).replace('.',',')}</span> </div>
            <div class="linha-resumo total"> <span>Total</span> <span>R$ ${total.toFixed(2).replace('.',',')}</span> </div>
        `;
        const resumoItensEl = document.getElementById('resumo-tela-itens');
        if (resumoItensEl) resumoItensEl.innerHTML = resumoHTML;
        const resumoEntregaEl = document.getElementById('resumo-tela-entrega');
        if (resumoEntregaEl) resumoEntregaEl.innerHTML = resumoHTML;
        const resumoPagamentoEl = document.getElementById('resumo-tela-pagamento');
        if (resumoPagamentoEl) resumoPagamentoEl.innerHTML = resumoHTML;
        const totalBotaoEl = document.getElementById('total-botao-carrinho');
        if (totalBotaoEl) totalBotaoEl.textContent = `R$ ${total.toFixed(2).replace('.', ',')}`;
    ...
    const gerenciarVisibilidadeFormEntrega = () => {
        ...
        const enderecoRuaInput = document.getElementById('endereco-rua');
        if (tipoEntrega === 'retirada') {
            ...
            if (enderecoRuaInput) enderecoRuaInput.required = false;
        } else {
            ...
            if (enderecoRuaInput) enderecoRuaInput.required = true;
        }
    };
