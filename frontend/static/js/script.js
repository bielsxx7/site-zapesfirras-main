...
        if (header && promoBanner) {
            const alturaTotal = header.offsetHeight + promoBanner.offsetHeight;
            const mainContent = document.querySelector('.container-principal');
            if(mainContent) {
                // Remove o padding-top do body e aplica no main container
                document.body.style.paddingTop = '0px'; 
                mainContent.style.paddingTop = `${alturaTotal}px`;
            }
        }
    }
...
    function mostrarNotificacao(mensagem, type = '') { if (!notificacao || !textoNotificacao) return; clearTimeout(timeoutNotificacao); notificacao.classList.remove('success', 'error'); if (type) { notificacao.classList.add(type); } textoNotificacao.textContent = mensagem; notificacao.classList.add('mostrar'); timeoutNotificacao = setTimeout(() => notificacao.classList.remove('mostrar'), 2500); }
...
    function popularAdicionais(produto) { if (!listaAdicionaisContainer || !toggleAdicionaisBtn) return; const categoria = produto.category_name || 'default'; const adicionaisDisponiveis = adicionaisPorCategoria[categoria] || adicionaisPorCategoria['default']; if (!adicionaisDisponiveis || adicionaisDisponiveis.length === 0) { toggleAdicionaisBtn.style.display = 'none'; listaAdicionaisContainer.innerHTML = ''; return; } toggleAdicionaisBtn.style.display = 'flex'; listaAdicionaisContainer.innerHTML = ''; adicionaisDisponiveis.forEach(adicional => { const adicionalHTML = ` <div class="item-adicional"> <label> <input type="checkbox" class="checkbox-adicional" data-nome="${adicional.name}" data-preco="${adicional.price}"> <span class="checkmark-adicional"></span> <span class="nome-adicional">${adicional.name}</span> </label> <span class="preco-adicional">+ ${formatCurrency(adicional.price)}</span> </div> `; listaAdicionaisContainer.insertAdjacentHTML('beforeend', adicionalHTML); }); }
    function atualizarPrecoTotalModal() { const quantidadeInput = document.querySelector('.modal-produto .entrada-quantidade'); const botaoAdicionar = document.querySelector('.botao-adicionar-carrinho-modal'); if (!quantidadeInput || !botaoAdicionar || !produtoAtualModal) return; const quantidade = parseInt(quantidadeInput.value); let precoTotalAdicionais = 0; document.querySelectorAll('.checkbox-adicional:checked').forEach(checkbox => { precoTotalAdicionais += parseFloat(checkbox.dataset.preco); }); const precoBase = parseFloat(produtoAtualModal.precoBase) || 0; const precoFinal = (precoBase + precoTotalAdicionais) * quantidade; produtoAtualModal.precoFinal = precoFinal / quantidade; botaoAdicionar.textContent = `Adicionar ${formatCurrency(precoFinal)}`; }
...
            const precoAdicionais = item.adicionais ? item.adicionais.reduce((sum, ad) => sum + ad.price, 0) : 0;
...
            }, 0);
            
            let taxaEntregaFinal = pedido.metodoEntrega === 'retirada' ? 0 : taxaDeEntrega;
            let desconto = 0;
            if (cupomAplicado && cupomAplicado.discount_type === 'free_delivery' && subtotal >= cupomAplicado.min_purchase_value) {
                desconto = taxaEntregaFinal;
                taxaEntregaFinal = 0;
...
        if (botaoPesquisa) { botaoPesquisa.addEventListener('click', (e) => { e.preventDefault(); const caixaPesquisa = botaoPesquisa.closest('.caixa-pesquisa'); const inputPesquisa = caixaPesquisa.querySelector('.texto-pesquisa'); caixaPesquisa.classList.toggle('ativo'); if (caixaPesquisa.classList.contains('ativo')) { inputPesquisa.focus(); } }); }
        if (barraFiltros) { barraFiltros.addEventListener('click', (e) => { if (e.target.classList.contains('botao-filtro')) { barraFiltros.querySelector('.ativo')?.classList.remove('ativo'); e.target.classList.add('ativo'); filtrarPorCategoria(e.target.dataset.categoria); } }); btnScrollLeft.addEventListener('click', () => barraFiltros.scrollBy({ left: -250, behavior: 'smooth' })); btnScrollRight.addEventListener('click', () => barraFiltros.scrollBy({ left: 250, behavior: 'smooth' })); barraFiltros.addEventListener('scroll', gerenciarSetasScroll); }
