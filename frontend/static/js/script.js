document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarDadosDaAPI() {
        try {
            ...
            if(containerPrincipal) {
                const secoesAntigas = containerPrincipal.querySelectorAll('.container-secao[data-category]');
                secoesAntigas.forEach(secao => secao.remove());
                containerPrincipal.insertAdjacentHTML('beforeend', '<p class="mensagem-erro-api">Não foi possível carregar o cardápio. Verifique se o servidor está rodando e tente novamente.</p>');
            }
        }
    }
    ...
    const atualizarQuantidade = (idUnico, novaQuantidade) => {
        ...
        if (itemIndex > -1) {
            // CORREÇÃO: Usar "quantity" (em inglês) ao invés de "quantidade"
    ...
    const atualizarTodosResumos = () => { const subtotal = carrinho.reduce((acc, item) => { const precoItemTotal = parseFloat(item.price) + (item.adicionais ? item.adicionais.reduce((sum, ad) => sum + ad.preco, 0) : 0); return acc + (precoItemTotal * item.quantity); }, 0); const tipoEntrega = document.querySelector('input[name="tipo-entrega"]:checked')?.value; const taxaEntrega = tipoEntrega === 'retirada' || carrinho.length === 0 ? 0 : TAXA_ENTREGA; const total = subtotal + taxaEntrega; const resumoHTML = ` <div class="linha-resumo"> <span>Subtotal</span> <span>R$ ${subtotal.toFixed(2).replace('.',',')}</span> </div> <div class="linha-resumo"> <span>Taxa de entrega</span> <span>R$ ${taxaEntrega.toFixed(2).replace('.',',')}</span> </div> <div class="linha-resumo total"> <span>Total</span> <span>R$ ${total.toFixed(2).replace('.',',')}</span> </div> `; document.getElementById('resumo-tela-itens').innerHTML = resumoHTML; document.getElementById('resumo-tela-entrega').innerHTML = resumoHTML; document.getElementById('resumo-tela-pagamento').innerHTML = resumoHTML; document.getElementById('total-botao-carrinho').textContent = `R$ ${total.toFixed(2).replace('.', ',')}`; const totalItens = carrinho.reduce((acc, item) => acc + item.quantity, 0); if (contadorCarrinhoMobileEl) { contadorCarrinhoMobileEl.textContent = totalItens; contadorCarrinhoMobileEl.classList.toggle('ativo', totalItens > 0); } if (contadorCarrinhoDesktopEl) { contadorCarrinhoDesktopEl.textContent = totalItens; contadorCarrinhoDesktopEl.classList.toggle('ativo', totalItens > 0); } };
    ...
    const finalizarEEnviarPedido = async () => { const tipoEntrega = document.querySelector('input[name="tipo-entrega"]:checked')?.value; const subtotal = carrinho.reduce((acc, item) => { const precoItemTotal = item.price + (item.adicionais ? item.adicionais.reduce((sum, ad) => sum + ad.preco, 0) : 0); return acc + (precoItemTotal * item.quantity); }, 0); const taxaEntrega = tipoEntrega === 'retirada' ? 0 : TAXA_ENTREGA; const total = subtotal + taxaEntrega; const deliveryInfo = tipoEntrega === 'retirada' ? { tipo: 'Retirada', rua: 'Retirar no local' } : { tipo: 'Entrega', rua: document.getElementById('endereco-rua').value, bairro: document.getElementById('endereco-bairro').value, numero: document.getElementById('endereco-numero').value, complemento: document.getElementById('endereco-complemento').value, }; const clientInfo = { nome: "Cliente do Site", telefone: "Não informado" }; const pedidoParaAPI = { client_info: clientInfo, delivery_info: deliveryInfo, items: carrinho, total_value: total, payment_info: pedido.pagamento, status: 'Novo' }; try { const response = await fetch('http://localhost:3000/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify(pedidoParaAPI), }); if (!response.ok) { throw new Error('Não foi possível enviar o pedido.'); } const result = await response.json(); console.log('Pedido enviado:', result); mostrarNotificacao(`Pedido #${result.orderId} enviado com sucesso!`); carrinho = []; salvarCarrinhoLocalStorage(); renderizarItensCarrinho(); togglePainelCarrinho(false); } catch (error) { console.error('Erro ao finalizar pedido:', error); mostrarNotificacao('Erro ao enviar pedido. Tente novamente.', 'error'); } };
    ...
    function configurarEventListeners() {
        ...
        document.querySelector('.botao-adicionar-carrinho-modal').addEventListener('click', () => {
            ...
            const produtoParaCarrinho = {
                ...
                name: produtoAtualModal.name, // CORRIGIDO: Era 'nome', o correto é 'name' para consistência
                price: parseFloat(produtoAtualModal.precoFinal),
                image: produtoAtualModal.image // CORRIGIDO: Era 'imagem', o correto é 'image'
            };
        ...
        btnContinuarCarrinho.addEventListener('click', () => {
            ...
            if (etapaAtualCarrinho === 'itens') {
                ...
                const formEndereco = document.getElementById('form-endereco'); // Adicionado para validação
                ...
                const metodo = document.querySelector('input[name="forma-pagamento-principal"]:checked').value;
                        if (metodo === 'pix') {
                            ...
                            if (precisaTroco === 'sim') {
                                const inputValorTroco = document.getElementById('valor-troco'); // Adicionado para pegar o valor
