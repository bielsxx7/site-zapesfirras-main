document.addEventListener('DOMContentLoaded', () => {
    ...
    const audioUnlockPrompt = document.getElementById('audio-unlock-prompt'); // <-- MODIFICAÇÃO
    ...
    // --- CONEXÃO SOCKET.IO E LISTENER DE NOVOS PEDIDOS ---
    const socket = io('http://localhost:3000');
    socket.on('new_order', (newOrder) => {
        console.log('Novo pedido recebido via WebSocket!', newOrder);
        
        // --- LÓGICA DE SOM MODIFICADA ---
        if (isAudioUnlocked) {
            notificationSound.play().catch(e => console.error("Erro ao tocar som de notificação:", e));
        } else {
            // Se o áudio NÃO estiver desbloqueado, mostra o aviso
            audioUnlockPrompt.classList.add('visible');
            console.warn('Áudio bloqueado. Mostrando aviso para interação do usuário.');
        }
    ...
    function applyTheme(theme) { state.theme = theme; document.body.classList.remove('light-mode', 'dark-mode'); document.body.classList.add(`${theme}-mode`); const themeToggle = document.getElementById('theme-toggle'); if (themeToggle) { themeToggle.checked = theme === 'dark'; } saveTheme(); }
    ...
    function renderOrderDetails(orderId) {
        ...
        const itemsHtml = order.items.map(item => `
            <div class="order-item-container">
                ...
                ${item.observacao ? `<div class="order-item-obs"><b>Obs:</b> ${item.observacao}</div>` : ''}
            </div>
        `).join('');
        ...
        if (order.payment_info.metodo === 'Dinheiro' && order.payment_info.trocoPara > 0) {
            ...
            paymentHtml += `<p><b>Pagar com:</b> ${formatCurrency(order.payment_info.trocoPara)}</p>`;
            if (change >= 0) {
                paymentHtml += `<p style="color:var(--primary-color); font-weight: bold;"><b>LEVAR TROCO: ${formatCurrency(change)}</b></p>`;
            } else {
                paymentHtml += `<p style="color: orange; font-weight: bold;"><b>ATENÇÃO: Valor do troco é menor que o total.</b></p>`;
            }
        } else if (order.payment_info.tipo) {
            paymentHtml += `<p><b>Detalhes:</b> ${order.payment_info.tipo}</p>`;
        }
        ...
        detailsColumn.innerHTML = `
            <div class="details-content">
                ...
                <div class="details-card-body"><p><b>Nome:</b> ${order.client_info.nome}</p></div>
                <div class="details-card-header"><ion-icon name="location-outline"></ion-icon>Entrega</div>
                <div class="details-card-body"><p><b>Tipo:</b> ${order.delivery_info.tipo}</p>${address}</div>
                <div class="details-card-header"><ion-icon name="fast-food-outline"></ion-icon>Itens</div>
                <div class="details-card-body">
                    ${itemsHtml}
                    <div class="details-total-row"><span>Total</span><span>${formatCurrency(order.total_value)}</span></div>
                </div>
    ...
    // --- LÓGICA PARA ADICIONAIS PERSONALIZADOS ---
    function renderCustomAddition(addition = { name: '', price: '' }) {
        ...
        row.innerHTML = `
            ...
            <div class="form-group">
                <input type="number" step="0.01" class="custom-addition-price" placeholder="Preço (R$)" value="${addition.price || ''}" required>
            </div>
            <button type="button" class="btn-remove-addition"><ion-icon name="trash-outline"></ion-icon></button>
        `;
        list.appendChild(row);
    }
    function setupCustomAdditionsListeners() {
        const container = document.getElementById('custom-additions-container');
        if (!container) return;

        document.getElementById('add-new-addition-btn').addEventListener('click', () => {
            renderCustomAddition();
        });

        document.getElementById('custom-additions-list').addEventListener('click', (e) => {
            const removeBtn = e.target.closest('.btn-remove-addition');
            if (removeBtn) {
                removeBtn.closest('.custom-addition-row').remove();
            }
        });

        document.querySelectorAll('input[name="adicionais-option"]').forEach(radio => {
    ...
    async function updateOrderStatus(orderId, newStatus) {
        ...
        if (order.delivery_info.tipo === 'Entrega' && oldStatus === 'Novo' && newStatus === 'Em Preparo') {
            if (!deliveryNumber) {
                deliveryNumber = incrementAndGetDeliveryNumber();
            }
        }
    ...
    // --- FUNÇÃO DE DESBLOQUEIO DE ÁUDIO MODIFICADA ---
    function unlockAudio() {
        ...
        audioUnlockPrompt.classList.remove('visible');
    }
    ...
    logoutButton.addEventListener('click', (e) => { e.preventDefault(); sessionStorage.removeItem('loggedInUser'); window.location.href = 'login.html'; });
