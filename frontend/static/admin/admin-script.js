document.addEventListener('DOMContentLoaded', () => {
    ...
    const audioUnlockPrompt = document.getElementById('audio-unlock-prompt'); // <-- CORRIGIDO: Seletor necessário
    ...
    // --- LÓGICA DE NOTIFICAÇÃO NO TÍTULO DA ABA ---
    let intervalId = null;
        ...
        intervalId = setInterval(() => {
            if (document.hasFocus()) {
                ...
                document.title = document.title === originalTitle 
                    ? `(${state.newOrdersCount}) Novo Pedido!` 
                    : originalTitle;
            }
        }, 1000);
    }
    ...
    // --- FUNÇÃO ÚNICA E CORRETA PARA LIBERAR O ÁUDIO ---
    function unlockAudio() {
        if (isAudioUnlocked) return;
        notificationSound.play().catch(() => {}).then(() => {
    ...
    socket.on('new_order', (newOrder) => {
        ...
        // --- LÓGICA DE SOM E AVISO RESTAURADA ---
        if (isAudioUnlocked) {
            notificationSound.play().catch(e => console.error("Erro ao tocar som:", e));
        } else {
            // Se o áudio NÃO estiver desbloqueado, mostra o aviso para o usuário
            if (audioUnlockPrompt) {
    ...
    function applyTheme(theme) { document.body.classList.remove('light-mode', 'dark-mode'); document.body.classList.add(`${theme}-mode`); const themeToggle = document.getElementById('theme-toggle'); if (themeToggle) { themeToggle.checked = theme === 'dark'; } saveTheme(); }
    ...
    // --- LÓGICA DE RENDERIZAÇÃO DE VIEWS (sem alterações) ---
...
function renderOrderDetails(orderId) {
    ...
    const address = order.delivery_info.rua === 'Retirar no local'
        ? '<p><b>Endereço:</b> Retirar no local</p>'
        : `<p><b>Endereço:</b> ${order.delivery_info.rua}, ${order.delivery_info.numero}</p>
           <p><b>Bairro:</b> ${order.delivery_info.bairro}</p>
           ${order.delivery_info.complemento ? `<p><b>Comp:</b> ${order.delivery_info.complemento}</p>` : ''}
           ${order.delivery_info.referencia ? `<p><b>Ref:</b> ${order.delivery_info.referencia}</p>` : ''}`;
    ...
    // Listener simplificado para desbloquear o áudio no primeiro clique
